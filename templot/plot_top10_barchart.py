...
def plot_top10_barchart(year, df, values, year_var, color_var, names_var,
                        title='', label='', ax=None, **kwargs):
    dff = df[df[year_var] == year].sort_values(
        by=values, ascending=True).tail(10)
    if ax is None:
        ax = plt.gca()
    ax.clear()
    l = list(dict.fromkeys(df[color_var]))
    if len(l) < 8:
        colors = dict(zip(l, [cm.Set2(x) for x in linspace(0, 0.87, len(l))]))
    else:
        colors = dict(
            zip(l, [cm.Set2(x) for x in linspace(0, 0.87, 8)] +
